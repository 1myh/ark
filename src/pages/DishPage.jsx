	...
	const DishPage = () => {
		...
		const {image, title, price, desc, titleColor, priceColor, buttonBackgroundColor = "#f74a26", buttonTextColor} = location.state || {};
			...
			<div className={`flex flex-col items-center`} >
				...
				<div className={`flex flex-col md:flex-row justify-center items-center md:items-start w-full`} >
					<div className={`flex order-2  flex-col max-w-[30rem] md:w-fit w-full px-10 py-5 my-5 ${language ? "items-end md:order-1" : "items-start md:order-2"}`} >
						<h1 className={`text-right text-3xl lg:text-5xl`} style={{color: titleColor}} >{title}</h1>
						...
						<h1 dir={language ? "rtl" : ""} className={`text-right text-2xl lg:text-4xl`} style={{color: priceColor}}>{price} {language ? "دينار" : "IQD"} </h1>
						<button onClick={handle} className={`lg:text-xl my-5 p-3 text-white w-fit rounded-lg`} style={{backgroundColor: buttonBackgroundColor, color: buttonTextColor}}> {language ? "اضافة الى الطلبية" : "add to cart"} </button>
					</div>
